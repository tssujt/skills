...
Generate worklog reports from Git commits, Claude Code sessions, Codex sessions, Kimi Code sessions, and Cursor sessions with AI summary.
...
- **Kimi Code Sessions** - reads `session_index.jsonl` and session state from `KIMI_CODE_HOME` (defaults to `~/.kimi-code`)
...
- Kimi Code Sessions (with time ranges)
